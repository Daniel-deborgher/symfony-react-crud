import React, { useState, useEffect } from 'react';
import { Link, useParams } from "react-router-dom";
import Layout from "../components/Layout"
import Swal from 'sweetalert2'
import axios from 'axios';
  
function VoitureEdit() {
    const [id, setId] = useState(useParams().id)
    const [model, setModel] = useState('');
    const [kmh, setKmh] = useState('')
    const [caracteristiques, setCaracteristiques] = useState('')
    const [isSaving, setIsSaving] = useState(false)
  
      
    useEffect(() => {
        axios.get(`/api/voiture/${id}`)
        .then(function (response) {
            let voiture = response.data
            setModel(voiture.model);
            setKmh(voiture.kmh);
            setCaracteristiques(voiture.caracteristiques);
        })
        .catch(function (error) {
            Swal.fire({
                icon: 'error',
                title: 'An Error Occured!',
                showConfirmButton: false,
                timer: 1500
            })
        })
          
    }, [])
  
  
    const handleSave = () => {
        setIsSaving(true);
        axios.patch(`/api/voiture/${id}`, {
            model: model,
            kmh: kmh,
            caracteristiques : caracteristiques
        })
        .then(function (response) {
            Swal.fire({
                icon: 'success',
                title: 'Voiture modifiée avec succès!',
                showConfirmButton: false,
                timer: 1500
            })
            setIsSaving(false);
        })
        .catch(function (error) {
            Swal.fire({
                icon: 'error',
                title: 'An Error Occured!',
                showConfirmButton: false,
                timer: 1500
            })
            setIsSaving(false)
        });
    }
  
  
    return (
        <Layout>
            <div className="container">
                <h2 className="text-center mt-5 mb-3">Modifier une voiture</h2>
                <div className="card">
                    <div className="card-header">
                        <Link 
                            className="btn btn-outline-info float-right"
                            to="/">Toutes les voitures
                        </Link>
                    </div>
                    <div className="card-body">
                        <form>
                            <div className="form-group">
                                <label htmlFor="model">Model</label>
                                <input 
                                    onChange={(event)=>{setModel(event.target.value)}}
                                    value={model}
                                    type="text"
                                    className="form-control"
                                    id="model"
                                    name="model"/>
                            </div>
                            <div className="form-group">
                                <label htmlFor="kmh">Kmh</label>
                                <input 
                                    value={kmh}
                                    onChange={(event)=>{setKmh(event.target.value)}}
                                    className="form-control"
                                    id="kmh"
                                    rows="3"
                                    name="kmh"></input>
                            </div>
                            <div className="form-group">
                                <label htmlFor="caracteristiques">Caractéristiques</label>
                                <textarea 
                                    value={caracteristiques}
                                    onChange={(event)=>{setCaracteristiques(event.target.value)}}
                                    className="form-control"
                                    id="caracteristiques"
                                    rows="6"
                                    name="caracteristiques"></textarea>
                            </div>
                            <button 
                                disabled={isSaving}
                                onClick={handleSave} 
                                type="button"
                                className="btn btn-outline-success mt-3">
                                Valider
                            </button>
                        </form>
                    </div>
                </div>
            </div>
        </Layout>
    );
}
  
export default VoitureEdit;